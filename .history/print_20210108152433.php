...
try {
    ...
    foreach ($_GET['produks'] as $item) {
        $printer->text("($item[jumlah]) Keterangan  : $item[keterangan]\n");
...
$mask = "|%5.5s |%-30.30s | x | x | x |\n";
printf($mask, 'Num', 'Title', 'ccc', 'ddd');
printf($mask, '1', 'A value that fits the cell', 'eeee', 'xxx');
printf($mask, '2', 'A too long value the end of which will be cut off', 'sdfsdf', 'ewrewrr');